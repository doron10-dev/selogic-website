import { siteUrls } from "@/data/contact";
import type { CardItem, CtaLink, FaqItem, HeroCtaLink } from "@/types/service-page";

export type MonitorItem = {
  label: string;
  sub: string;
};

export type RmmContent = {
  hero: {
    eyebrow: string;
    title: string;
    intro: string;
    primaryCta: HeroCtaLink;
    secondaryCta: HeroCtaLink;
  };
  /** מסר מרכזי: מזהים · מטפלים · מדווחים · משפרים */
  flow: {
    title: string;
    body: string;
    steps: { title: string; body: string }[];
  };
  scope: {
    title: string;
    body: string;
    note: string;
    items: MonitorItem[];
  };
  value: {
    title: string;
    body: string;
    items: CardItem[];
  };
  reports: {
    title: string;
    body: string;
    reportTitle: string;
    reportItems: string[];
    kpiTitle: string;
    kpiItems: string[];
    disclaimer: string;
  };
  faq: {
    title: string;
    body: string;
    items: FaqItem[];
  };
  finalCta: {
    title: string;
    body: string;
    primary: CtaLink;
    secondary: CtaLink;
  };
};

export const rmmPage: RmmContent = {
  hero: {
    eyebrow: "ניטור ובקרה RMM",
    title: "מזהים בעיות לפני שהן עוצרות את העסק",
    intro:
      "ניטור שוטף ברקע של רכיבי סביבת המחשוב, כדי לזהות חריגות מוקדם, לטפל מרחוק, לתעד כל פעולה ולהציג לכם תמונת מצב ברורה. אצל סלוג׳יק RMM הוא לא רק כלי טכני, אלא שירות ניהולי שמכניס סדר, שקיפות ובקרה לסביבת המחשוב.",
    primaryCta: { label: "קבעו שיחת אבחון", href: siteUrls.contactDiagnosis },
    secondaryCta: { label: "פתחו קריאת שירות", href: siteUrls.technicalSupport },
  },
  flow: {
    title: "איך הניטור עובד בפועל",
    body: "ארבעה שלבים שחוזרים על עצמם, כדי שסביבת המחשוב תישאר תחת שליטה לאורך זמן.",
    steps: [
      { title: "מזהים מוקדם", body: "ניטור מתמשך של רכיבי הסביבה, כדי לזהות חריגות לפני שהן הופכות להשבתה." },
      { title: "מטפלים מרחוק", body: "כשאפשר, מטפלים בחריגה מרחוק במהירות, בלי להמתין להגעה פיזית." },
      { title: "מדווחים ללקוח", body: "מה שנמצא ומה שטופל מתועד ונכנס לתמונת המצב ולדוח התקופתי." },
      { title: "משפרים לאורך זמן", body: "מזהים מגמות ופערים חוזרים ומתקנים את השורש, לא רק את התקלה." },
    ],
  },
  scope: {
    title: "מה מנטרים ברקע",
    body: "ניטור מתמשך של הרכיבים שמחזיקים את העבודה השוטפת, עם התראה על חריגות.",
    note: "ניטור שוטף ברקע",
    items: [
      { label: "מחשבים", sub: "מצב תחנות עבודה וזמינות" },
      { label: "שרתים", sub: "עומסים, שירותים ומשאבים" },
      { label: "גיבויים", sub: "האם הגיבוי רץ והושלם" },
      { label: "עדכונים", sub: "מצב עדכוני מערכת ואבטחה" },
      { label: "אבטחת מידע", sub: "רכיבי הגנה והתראות" },
      { label: "תחנות קצה", sub: "מצב EDR או אנטי וירוס" },
      { label: "שירותים קריטיים", sub: "תהליכים שחייבים לרוץ" },
      { label: "רשת ותקשורת", sub: "קישוריות ורכיבי תקשורת" },
      { label: "אירועים פתוחים", sub: "חריגות שממתינות לטיפול" },
      { label: "רכיבים בסיכון", sub: "רכיבים שדורשים תשומת לב" },
    ],
  },
  value: {
    title: "מה זה נותן ללקוח",
    body: "פחות הפתעות, יותר סדר ותמונת מצב אמיתית על סביבת המחשוב.",
    items: [
      { title: "זיהוי מוקדם", body: "חריגות עולות לטיפול לפני שהן מגיעות למשתמשים." },
      { title: "פחות הפתעות", body: "פחות תקלות שמגיעות בלי התראה ועוצרות את העבודה." },
      { title: "טיפול מרחוק", body: "חלק גדול מהטיפול נעשה מרחוק ובמהירות." },
      { title: "תיעוד פעולות", body: "כל פעולה מתועדת ונשמרת כחלק מתיק הלקוח." },
      { title: "התראות", body: "התראה על חריגות ברכיבים שמוגדרים כחשובים." },
      { title: "שקיפות", body: "אתם רואים מה מנוטר, מה נמצא ומה טופל." },
      { title: "דוחות", body: "תמונת מצב תקופתית שמסכמת מה קרה בסביבה." },
      { title: "בסיס ל-KPI", body: "בסיס למדדי תחזוקה והגנה לאורך זמן." },
    ],
  },
  reports: {
    title: "דוחות ו-KPI שמראים מה קורה באמת",
    body: "סלוג׳יק לא רק מטפלת בתקלות — היא מודדת, מדווחת ומשפרת. כך נראים המבנה של דוח תקופתי וסוגי המדדים שאפשר לעקוב אחריהם.",
    reportTitle: "מבנה דוח תקופתי",
    reportItems: [
      "קריאות שנפתחו",
      "קריאות שנסגרו",
      "קריאות פתוחות",
      "קריאות חוזרות",
      "אירועי אבטחת מידע",
      "מצב גיבויים",
      "מצב עדכונים",
      "פערים פתוחים",
      "סיכונים",
      "המלצות להמשך",
    ],
    kpiTitle: "סוגי מדדי KPI",
    kpiItems: [
      "גיבויים",
      "עדכונים",
      "תחנות מנוטרות",
      "שרתים",
      "הרשאות",
      "EDR או אנטי וירוס",
      "אירועי אבטחה",
      "קריאות חוזרות",
      "תיעוד",
      "סיכונים פתוחים",
    ],
    disclaimer: "להמחשה בלבד. הנתונים מוצגים לאחר חיבור לסביבת הלקוח.",
  },
  faq: {
    title: "שאלות נפוצות על ניטור ובקרה",
    body: "מה שחשוב לדעת לפני שמתחילים.",
    items: [
      {
        q: "מה זה RMM בעצם?",
        a: "כלי ניהול וניטור מרחוק שמאפשר לנו לראות את מצב רכיבי הסביבה, לזהות חריגות ולטפל מוקדם, בלי להמתין שתיתקלו בתקלה. אצלנו הוא מנוהל כשירות, לא רק כטכנולוגיה.",
      },
      {
        q: "האם הניטור מחליף גיבוי או אבטחת מידע?",
        a: "לא. הניטור משלים אותם. הוא עוקב אחרי מצב הגיבויים ורכיבי האבטחה ומתריע על חריגות, אבל הוא חלק ממערך רחב יותר של שירות מנוהל.",
      },
      {
        q: "מה קורה כשמזוהה חריגה?",
        a: "אנחנו מתעדים אותה, בודקים, מטפלים מרחוק כשאפשר ומעדכנים אתכם. מה שרלוונטי נכנס לתמונת המצב ולדוח התקופתי.",
      },
      {
        q: "איך אני רואה מה קורה בסביבה שלי?",
        a: "דרך תמונת מצב ודוח תקופתי שמסכמים מה נמצא ומה טופל. בהמשך אפשר גם דרך פורטל הלקוח.",
      },
    ],
  },
  finalCta: {
    title: "רוצים לראות את סביבת המחשוב שלכם בצורה מסודרת?",
    body: "נתחיל בשיחת אבחון קצרה, נבין מה חשוב לכם, ונראה מה אפשר להכניס לניטור ובקרה.",
    primary: { label: "קבעו שיחת אבחון", href: siteUrls.contactDiagnosis, sub: "נחזור אליכם תוך יום עסקים אחד" },
    secondary: { label: "פתחו קריאת שירות", href: siteUrls.technicalSupport, sub: "לקוחות קיימים שצריכים טיפול" },
  },
};
