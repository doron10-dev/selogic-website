...
function DomainNode({ label, icon: Icon, index }: { label: string; icon: LucideIcon; index: number }) {
  return (
    ...
    >
      <span
        className={`mx-auto block h-4 w-px bg-blue-400/35 ${motionStyles.drawY}`}
        style={{ animationDelay: `${500 + index * 100}ms` }}
      />
      <div className="flex h-[4.75rem] w-full flex-col items-center justify-center gap-2 rounded-xl border border-slate-800 bg-slate-900/90 px-1.5 text-center shadow-sm">
        <Icon className="h-4 w-4 shrink-0 text-blue-300/80" aria-hidden="true" />
        <span className="text-[10px] font-medium leading-tight text-slate-300 sm:text-[11px]">{label}</span>
      </div>
    </li>
  );
}
...
export function ManagedOwnershipNode() {
  return (
    <div className="mx-auto w-full max-w-sm overflow-hidden px-1 py-4" aria-hidden="true">
      <div
        className={`relative z-10 mx-auto flex w-max max-w-full items-center gap-3 rounded-full border border-blue-400/30 bg-slate-950 px-5 py-2.5 shadow-sm ${motionStyles.riseFromTop}`}
      >
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-blue-400/15">
          <Settings className="h-3.5 w-3.5 text-blue-300" />
        </span>
        <span className="truncate text-sm font-semibold text-slate-100">כתובת אחת אחראית</span>
      </div>

      <div
        className={`mx-auto h-5 w-px bg-blue-400/40 ${motionStyles.drawY}`}
        style={{ animationDelay: "200ms" }}
      />
      <div
        className={`mx-[10%] h-px bg-blue-400/35 ${motionStyles.drawX}`}
